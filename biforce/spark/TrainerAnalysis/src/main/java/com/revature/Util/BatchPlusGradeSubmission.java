...
public class BatchPlusGradeSubmission {
	public static void calcGradeSubmission(JavaSparkContext context, SparkSession session, String input_batch, String input_assessment, String input_note, String output) {
        ...
        Dataset<Row> data_batch = session.sqlContext().read().format("csv").schema(schema).option("delimiter", "~").option("header", "false").option("inferSchema", "false").option("mode", "PERMISSIVE").option("nullValue", "null").load(input_batch);
		...
		Dataset<Row> data_assessment = session.sqlContext().read().format("csv").option("delimiter", "~").option("header", "false").option("inferSchema", "false").option("mode", "PERMISSIVE").option("nullValue", "null").schema(schema).load(input_assessment);
        ...
        schema = DataTypes.createStructType(fields);
		Dataset<Row> data_note = session.sqlContext().read().format("csv").option("delimiter", "~").option("header", "false").option("inferSchema", "false").option("mode", "PERMISSIVE").option("nullValue", "null").schema(schema).load(input_note);
