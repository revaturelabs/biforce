 ...
 * <p>
 * This class consists exclusively of a main method that interacts with
 * utilities which interact with a logistic regression model. It has several
 * helper methods which write output. All methods are static.
 * </p>
 * <p>
 * The model returns percentage values which indicate likelihood to fail and
 * then runs through all possible cutoff values to determine the optimal cutoff
 * percentage.
 * </p>
 * <p>
 * The model clumps trainees together in groups for each of the first few weeks
 * by their test scores into buckets based on equi-distant percentiles. The
 * model then calculates the odds of someone in that bucket passing or failing
 * and then calculates the log odds taken from this tutorial.
 * <a href="http://vassarstats.net/logreg1.html">Logistic Regression</a>.
 * </p>
 * <p>
 * TODO: describe the rest of log-reg logic.
 * </p>
 * <p>
 * This class is a member of the
 * <a href="https://github.com/Dec-17-Big-Data/biforce"> Revature Biforce</a>
 * Github project.
 * </p>
 * <p>
 * For more information regarding this project see the <a href=
 * "https://drive.google.com/open?id=1xD-x-0oX2vXWdEpoMhq0Jpu0j2gCgpjOufVh5NerJHQ">
 * Biforce Living Document</a>.
 * </p>
 * <h2>Used columns</h2>
 * <ul>
 * <li>_c0 = PK</li>
 ...
 * <li>_c1 = 2 : A.ASSESSMENT_TYPE - 'VERBAL' = 1; 'EXAM' = 2; - 'PROJECT' = 3;
 * 'OTHER' = 4</li>
 ...
 * <li>_c8 = 2 : G.SKILL_TYPE - 'SDET' = 1; 'J2EE' = 2; 'OTHER' = 3; - 'BPM' =
 * 4; 'NET' = 5; 'MICROSERVICES' = 6</li>
 * <li>_c9 = 281214 : Q.TRAINEE_ID</li>
 * <li>_c10 = 2 : B.TRAINING_STATUS - 'DROPPED'= 0; 'EMPLOYED' = 1; 'TRAINING' =
 * 2; - 'SIGNED' = 3; 'CONFIRMED' = 4; 'MARKETING' = 5</li>
 * </ul>
 * 
 * @author Mason Wegert
 * @author Diego Gomez
...
public class Driver {
	 ...
	 * Then it calls a plethora of utility functions. Data flow includes splitting
	 * control & model data, training the model, testing the model, and printing the
	 * results.
	 * 
	 * @param args - 0 input file location, 1 is main output, 2 is model parameters
	 *             output
	 */
	public static void main(String args[]) {
		...
		// Note that associate status (c10) is consistent across all test weeks as it's
		...
		modelData.persist(); // holds modeldata in memory so it doesn't have to repeat the above
								// filters/joins (transformations)
		double[][] modelParams = ModelFunction.execute(modelData, PartitionFinder.read(modelData, modelSplitCount),
				modelSplitCount);
		modelData.unpersist();
		...
		// Assumed controlRDD is testing test data with model already applied to third
		// column
		...
		// Applies model onto the entire dataset and writes to output
	...
	/**
	 * It prints the equation for calculating the probability of failure based on
	 * the modelParams. Prints to the console and accuracyWriter for each of the 3
	 * exam types (one for verbal, exam, project scores). Test type 4 (other) has a
	 * low correlation (0.2) and negatively effects the results.
	 * 
	 * @param modelParams
	 */
	private static void printModel(double[][] modelParams) {
		for (int i = 0; i < 3; i++) {
			String s = String.format(
					"Exam type " + (i + 1)
							+ ": partialFailChance = e^(%2.3f*score+%2.3f) / (1+e^(%2.3f*score+%2.3f), r^2 = %1.3f\n",
	...
	/**
	 * Writes the controlRDD output to the accuracy writer. If a row's drop % is
	 * greater than dropPercent it writes 'DROP', otherwise it writes 'PASS' at the
	 * end of the line.
	 * 
	 * @param controlRDD
	 * @param dropPercent
	...
	private static void writeOutput(JavaPairRDD<Integer, Row> appliedResultPair, double dropPercent) {
		appliedResultPair.foreach(pairTuple -> {
			...
			writer.append(pairTuple._1 + "," + pairTuple._2.getDouble(1) / pairTuple._2.getDouble(2) + ","
					+ pairTuple._2.getInt(3) + "," + prediction + "\n");
		});
	}
	...
	private static OptimalPoint applyControl(JavaRDD<Row> controlRDD, double accuracyDelta, int weekNum) {
		...
		writeToControl(
				"Fail percent: " + Math.round(optimalPoint.getOptimalPercent() * 10000) / 10000.0
						+ "\nCorrect estimates: " + optimalPoint.getOptimalAccurateCount() + "\nTotal Count: "
						+ controlRDD.count() + "\nAccuracy: "
	...
	/**
	 * a simple writer class to the accuracyWriter and console, outString gets
	 * written/appended.
	 * 
	 * @param outString
	 */
