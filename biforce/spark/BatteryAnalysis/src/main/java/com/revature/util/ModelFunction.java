...
//		model[6] = logReg(statsDS(binDS(modelDS(csv, "_c4", 3))), 7);
	...
	private static Dataset<Row> modelDS(Dataset<Row> data, String modelType, int intType) {
		...
		//Dataset<Row> result = modelTypeDS.join(status).where(modelTypeDS.col("_c9").$eq$eq$eq(status.col("assc1"))).select("_c9", "avg_score", "status");
		return result;
	...
	private static Dataset<Row> binDS(Dataset<Row> input, List<Double> partitions) {
		input.persist();
		Dataset<Row> bins = input.filter("avg_score < 11").withColumn("bin", functions.lit(1));
		int binNum = 2;
		for(int i = 11; i <= 91; i+=10){
			Dataset<Row> bin = input.filter("avg_score >= " + i + " and avg_score < "+ (i + 10)).
	...
	//stats DS takes the bulk of the processing time
	...
	private static double[] logReg(double[][] stats, int modelNum) {
		...
		return new double[]{modelNum, m, b, r2};
	}
}